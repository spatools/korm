...
import dataset = require("../src/dataset");
import relations = require("../src/relations");
...
export class FakeDataAdapter implements adapters.IAdapter {
    ...
    public getAll(controller: string, query?: query.ODataQuery): Promise<adapters.IAdapterResult> {
        return this.getStore(controller).then(store => {
            var values = _.values(store),
                result: adapters.IAdapterResult = {
                    data: values,
                    count: values.length
                };

            if (query) {
                result.data = query.apply(result.data);


                if (query.selects.size() > 0) {
                    result.data = this.applySelectsRange(result.data, query.selects());
                }

                if (query.expands.size() > 0) {
                    return this.applyExpandsRange(controller, query.expands(), result.data).then(() => result);
                }
            }
...
export function initDataContext(store?: stores.IDataStore, adapter?: adapters.IAdapter, force?: boolean): Promise<any> {
    ...
    if (store)
        promises.push(datacontext.setLocalStore(store));
